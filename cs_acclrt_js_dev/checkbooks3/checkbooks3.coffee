...
mainStep = ->
  inquirer.prompt(promptFactory.accountPrompt()).then(({account})->
    if account is null
      createAccountStep()
    else
      actionStep({account})
...
createAccountStep = ->
  inquirer.prompt(promptFactory.newAccountPrompt()).then(({name})->
...
actionStep = ({account})->
  inquirer.prompt(promptFactory.actionPrompt({account})).then(({action})->
